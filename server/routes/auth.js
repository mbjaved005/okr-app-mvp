...
router.post('/register', upload.single('profilePicture'), async (req, res) => {
  ...
  try {
    ...
    const validDepartments = ['QA', 'Frontend', 'Backend', 'PM', 'HR', 'Marketing', 'Design', 'DevOps', 'Operations', 'Network'];
    ...
    const user = await UserService.createUser({ name, email, password, role, designation, department, profilePicture });
    log.info(`User registered successfully: ${user.email}`);
    const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, { expiresIn: '1d' });
    console.log("User data being sent to client after registration:", user.toJSON());
...
router.post('/update-profile-picture', requireUser, upload.single('profilePicture'), async (req, res) => {
  try {
    ...
    log.info(`User profile picture updated successfully: ${updatedUser.email}`);
    console.log("Updated user data:", updatedUser.toJSON());
    return res.json({
      success: true,
    ...
    return res.status(500).json({ error: 'An unexpected error occurred while updating the profile picture' });
  }
});
