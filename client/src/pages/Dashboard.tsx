...
export function Dashboard() {
  ...
  useEffect(() => {
    const filtered = okrs.filter((okr) => {
      const matchesCategory =
        filters.category === "all" || okr.category === filters.category;
      const matchesDepartment =
        filters.department === "all" || okr.department === filters.department;

      const okrDate = new Date(okr.startDate);
      const quarter = Math.floor((okrDate.getMonth() + 3) / 3);
      const matchesQuarter =
        filters.quarter === "all" || `Q${quarter}` === filters.quarter;
      ...
      const matchesDateRange =
        (!startDate || okrStartDate >= startDate) &&
    ...
    const filteredUsersByDepartment = users.filter(
      (user) =>
  ...
  const calculateAverageProgressForMonth = (filteredOkrs, month) => {
    ...
    return Math.round(
      okrsInMonth.reduce((sum, okr) => sum + okr.progress, 0) /
        okrsInMonth.length
    );
  };
  ...
  const calculateAverageProgress = () => {
    ...
    const totalProgress = filteredOkrs.reduce(
      (sum, okr) => sum + okr.progress,
      0
    );
