...
import { Plus, Filter, Search, RotateCcw, Edit, Trash2, Eye } from "lucide-react"
...
export function OKRs() {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const updatedOkrs = okrToDelete ? okrs.filter(okr => okr._id !== okrToDelete._id) : okrs
      setOkrs(updatedOkrs)
      toast({
        title: "Success",
          ...
          <div className="space-y-2">
            {filteredOKRs.length > 0 ? (
              filteredOKRs.map((okr) => (
                <Card key={okr._id} className="hover:shadow-lg transition-shadow bg-transparent border-none p-1">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col justify-between h-12">
                          <CardTitle className="text-lg text-foreground">{okr.title}</CardTitle>
                          <p className="font-lg text-foreground">{okr.department} | {okr.category}</p>
                        </div>
                      </div>
